...
public class TestingWebApplicationTests {
    ...
    public void findRecipeByNameReturnsNotFoundExc() throws Exception {
        this.mockMvc.perform(get(Constants.BASE_API_PATH + "/recipes/search/Jr"))
                .andExpect(status().isNotFound());
    }

    @Test
    public void findRecipeByNameReturnsListOfRecipes() throws Exception {
        Recipe recipe = Recipe.builder().id(2L).author(User.builder().uid("admin").id(1L).build()).name("Не очень горячая курица").cookTimeMins(40).build();
        Recipe recipe2 = Recipe.builder().id(3L).author(User.builder().uid("admin").id(1L).build()).name("Очень-очень горячая курица").cookTimeMins(30).build();
        List<Recipe> recipeList = Arrays.asList(recipe, recipe2);
        when(recipeRepository.findByNameContaining("Очень", 0))
                .thenReturn(recipeList);

        this.mockMvc.perform(get(Constants.BASE_API_PATH + "/recipes/search/Очень"))
                .andExpect(status().isOk())
                .andExpect(content().contentType("application/json"))
                .andExpect(jsonPath("$[0].id", is(2)))
                .andExpect(jsonPath("$[0].author_uid", is("admin")))
                .andExpect(jsonPath("$[0].name", is("Не очень горячая курица")))
                .andExpect(jsonPath("$[0].cook_time_mins", is(40)))
                .andExpect(content().contentType("application/json"))
                .andExpect(jsonPath("$[1].id", is(3)))
                .andExpect(jsonPath("$[1].author_uid", is("admin")))
                .andExpect(jsonPath("$[1].name", is("Очень-очень горячая курица")))
                .andExpect(jsonPath("$[1].cook_time_mins", is(30)));
    }

    @Test
    public void deleteRecipeByIdUnauthorized() throws Exception {
        this.mockMvc.perform(delete(Constants.BASE_API_PATH + "/recipes/2"))
    ...
    public void PostRecipeUnauthorized() throws Exception {
        ...
        when(userRepository.findByUid("user"))
                .thenReturn(Optional.of(User.builder().uid("user").id(2L).build()));
        Authentication auth = SecurityTestUtils.createAuthenticationWithRoles(roles, "user");
        RecipeDto recipeDto = new RecipeDto()
                .id(1L)
                .name("Super dish")
                .cookTimeMins(3)
                .ingredientsDistributions(Collections.singletonList(new IngredientsDistributionDto().ingredientId(3L).name("salt").measureUnitName("th")))
                .authorUid("user")
                .media(new IdDto().id(3L))
                .steps(Collections.singletonList(new StepDto().stepNum(1).description("some")));
        Recipe recipe = Recipe.builder().id(1L).author(User.builder().uid("user").id(2L).build()).name("Super dish").cookTimeMins(30).build();

        mockMvc.perform(post(Constants.BASE_API_PATH + "/recipes")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(asJsonString(recipeDto)).with(authentication(auth)))
